...
function PropertyShell() {
  ...
  const breadcrumb = (
    <div style={{ fontSize: 13, color: COLORS.secondary }}>
      <Link to="/fastigheter" style={{ color: COLORS.secondary, textDecoration: "none" }}>Fastigheter</Link>
      {" › "}
  ...
  return (
    <PropertyProvider value={{ property, propertyId: id }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16, fontFamily: "Inter, system-ui, sans-serif", padding: isMobile ? 16 : 32 }}>
          ...
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
              ...
              <div style={{ textAlign: "right", minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: COLORS.text, fontFamily: "Outfit, Inter, system-ui, sans-serif" }}>
                  {property?.name ?? "…"}
                </div>
                {property?.address && (
                  <div style={{ fontSize: 12, color: COLORS.secondary, marginTop: 2 }}>{property.address}</div>
                )}
              </div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <h1 style={{ fontSize: 24, fontWeight: 700, color: COLORS.text, margin: 0 }}>Kontakter</h1>
              <Link
                to="/contacts/new"
                style={{
                  display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 18px",
                  background: "#5CB84A", color: "#fff", borderRadius: 999, textDecoration: "none",
                  fontSize: 14, fontWeight: 600, fontFamily: "Outfit, Inter, system-ui, sans-serif", whiteSpace: "nowrap",
                }}
              >
          ...
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
              <div>
                ...
                {property?.address && (
                  <div style={{ fontSize: 13, color: COLORS.secondary, marginTop: 4 }}>{property.address}</div>
                )}
              </div>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: isMobile ? 16 : 24, boxShadow: "0 1px 2px rgba(13,43,30,0.03), 0 8px 24px -14px rgba(13,43,30,0.10)" }}>
          <Outlet />
        </div>
      </div>
    </PropertyProvider>
